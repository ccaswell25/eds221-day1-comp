...
### MY NOTES:
## COMMAND-SHIFT-M will automatically enter the pipe operator
## When adding the pipe operator we're saying after reading in the data, we want to clean the names. The pipe operator allows us to do multiple actions in one assignment. 
## Use janitor clean names to make all column names in lower snake case
## How do we fix the N/As in the dataset? we can add NA information in the read_csv() section (see above)

### SAVING AS A FILE

```{r}

## Creating a jitter chart with a color gradient:
my_awesome_chart <- ggplot(data = toolik_weather, aes(x = month, y = daily_air_temp_mean_c, color = daily_air_temp_mean_c)) +
  geom_jitter() + 
  labs( x = "Month", y = "Daily Mean Air Temperatures (Celsius)") + 
  scale_color_gradient(low = "red", high = "blue")

## How do we save this as a component of our working directory aka a file? We can use ggsave and add the name of what we want to save and use the here function with the path we want to save it in followed by the name and type of file we want to give it (in this case it's a .png). Adding height and width we can give it a file size as well. 
ggsave(plot = my_awesome_chart, here::here("figs", "my_awesome_chart.png"), height = 5, width = 6)
